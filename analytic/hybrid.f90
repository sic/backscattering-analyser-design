...
! Define terms in equation Ax^2+Bx+Cy^2+Dy+Ez^2+Fz+G+Pxy+Qyz+Rxz=0!	A = (cp*cp)/(ar*ar) + (sp*sp)/(br*br)!	B = ((-2*s*cp*cp)/(ar*ar)) + ((-2*t*cp*sp)/(ar*ar)) + &!	    ((-2*s*sp*sp)/(br*br)) + ((-2*t*cp*sp)/(br*br))!	C = (sp*sp)/(ar*ar) + (cp*cp)/(br*br)!	D = ((-2*t*sp*sp)/(ar*ar)) + ((-2*t*cp*cp)/(br*br)) + &!	    ((-2*s*cp*sp)/(ar*ar)) + ((-2*s*cp*sp)/(br*br))!	E = 0!	F = 0!	G = ((s*s*cp*cp)/(ar*ar)) + ((t*t*sp*sp)/(ar*ar)) + &!	    ((s*s*sp*sp)/(br*br)) + ((t*t*cp*cp)/(br*br)) + &!	    ((2*s*t*cp*sp)/(ar*ar)) + ((-2*s*t*cp*sp)/(br*br)) - 1!	P = (2*cp*sp)/(ar*ar) + (-2*cp*sp)/(br*br)!	Q = 0!	R = 0!!	result = (A*sa**2) + (B*sa) + G!	WRITE(*,*) ' This figure should be close to zero --> ',result	                            !	WRITE(*,*) ' Axes of ellipse are ',ar,br	x(:) = sa-(sa/5)                                                                      ! if we know Y then we have an equation of the form qa*X^2 + qb*X + qc = 0      DO i = 1, n 	  y(i) = ystart - (i-1) - 0.5	  count = 0	  unit = 50!	  qa = A!	  qb = B + (P*y(i))!	  qc = (C*y(i)**2) + (D*y(i)) + G	  DO	   x(i) = x(i) + 0.001	   count = count + 1	   path = sqrt(x(i)**2 + y(i)**2) + & 		   sqrt((x(i)-xd)**2 + (y(i)-yd)**2)	   !WRITE(*,*) x(i), count, path 	   IF (abs(path-pathc)<0.001) EXIT	  END DO	  gradient = ((yd+2.5)/xd)*x(i)	  write(6,*) '====================================='	  IF (gradient > y(i)) THEN		WRITE(6,*) ' WARNING - Analyser Element &		&is being shielded by detector'	  unit = 51	  ENDIF!	  DO !         lambdac = 6.69 * sin((3.14159265359/2.0) - phi)! 	   IF (abs(ttheta-tthetac)<0.001) EXIT!        END DO	  write(6,*) ' Analyser Crystal No : ',i!SIC	  phi(i) =  -atan((y(i)-yo)/(x(i)-xo))/0.01745329251		  tmp = atan(y(i)/x(i))/0.01745329251        phi(i) = -(180.0 - (90.0 - tmp) - ttheta/2.0)		  write(6,100) x(i), y(i), phi(i)	  write(6,*) ' Analysed wavelength = ',6.69*sin(tmp)	  write(6,*) ' Secondary Path Length = ',path	  write(unit,100) x(i), y(i), phi(i)	ENDDO      100	FORMAT(1X,F7.3,1X,F7.3,1X,F7.3) 	CLOSE(50)	CLOSE(51)  END PROGRAM hybrid
